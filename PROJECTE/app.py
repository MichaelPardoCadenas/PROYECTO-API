...
from bd import obtener_jugadores, obtener_equipos, obtener_estadisticas # Asegúrate de que esta función esté definida en bd.py
from auth import require_apikey, generar_y_guardar_apikey   # Importar el decorador de autenticación
...
def generar_apikey():
    ...
    if not id_usuario:
        return jsonify({'message': 'Se requiere un ID de usuario'}), 400

    nueva_apikey = generar_y_guardar_apikey(id_usuario)
    
    return jsonify({'message': 'Clave API generada exitosamente', 'api_key': nueva_apikey})
